...
class Blockchain {
  ...
  addBlock(newBLock, state) {
    ...
    if (state == false) {
      newBLock.Transactions = this.pendingTransactions.slice(
        0,
        this.maxTransperblock
      );
    } else {
  ...
  isChainValid() {
    ...
    for (let i = 1; i < this.chain.length; i++) {
      ...
      if (
        currentblock.hash !==
        currentblock.calculateHash() /*|| checkminingstat(currentblock.hash)*/
      ) {
        return false;
      }
  ...
  DoesVoteExist(uid) {
    ...
    if (
      this.getLatestblock().Transactions == "Jaival Faisal Bautik" &&
